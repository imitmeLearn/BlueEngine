...
bool ModelLoader::Load(const std::string & name,std::weak_ptr<MeshData>& outData)
{
	...
	//앞에서 구한 탄젠트,바이탄젠트, 노멀 의 직교성 보장하도록 계산.
	//그람-슈미트 알고리즘 & 외적.
	...
	std::shared_ptr<MeshData> newData=std::make_shared<MeshData>(vertices,indices);	//메시 데이터 생성 및 리소스 등록
